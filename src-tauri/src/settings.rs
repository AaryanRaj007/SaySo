...
/// and `Dark` force one of the two palettes SaySo already ships.
#[derive(Serialize, Deserialize, Debug, Clone, Copy, PartialEq, Eq, Type)]
#[serde(rename_all = "snake_case")]
pub enum Theme {
    System,
...
/* still sayso for composing the initial JSON in the store ------------- */
...
fn default_update_checks_enabled() -> bool {
    // Off by default: SaySo has no updater endpoint configured, and the
    // `pubkey` in tauri.conf.json is still upstream Handy's minisign key, whose
    // private half we don't hold. Until both are replaced an update check can
    // only ever fail, so the footer says "disabled" rather than offering a
    // button that errors. Flip back to `true` once endpoints + our own keypair
    // are in place.
    false
}
